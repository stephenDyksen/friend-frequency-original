/*
 official_game_backend.php responds to http calls by connecting to the database and modifying or returning data to the caller
...
<?php
    /*
     * official_game_backend.php responds to requests to GameTbl table in friendfrequency database
     */
    ...
    function doPost()
    {
        ...
        $result = json_encode(mysql_query("INSERT INTO GameTbl(fb_id_a,fb_id_b,words_a,words_b, timestamp) VALUES(" . $values . ")"));
    ...
    function doPut()
    {
        ...
        $result = json_encode(mysql_query("UPDATE GameTbl SET " . $values . " WHERE fb_id_a=" . $GLOBALS['least_fb_id'] . " AND fb_id_b=" . $GLOBALS['greatest_fb_id']));
    ...
    function doGet()
    {
        ...
        if($_SERVER['HTTP_GET_ALL'] == "FALSE"){
            $result = mysql_query("SELECT * FROM GameTbl WHERE fb_id_a=" . $GLOBALS['least_fb_id'] . " AND fb_id_b = " . $GLOBALS['greatest_fb_id']);
//            $result_arr[] = mysql_fetch_assoc($result);
            //$result = array item if successful
            //          false if resource was not present in DB
        }else{
            $result = mysql_query("SELECT * FROM GameTbl WHERE fb_id_a=" . $GLOBALS['least_fb_id'] . " OR fb_id_b = " . $GLOBALS['least_fb_id']);
        }
    ...
    function doDelete()
    {
        $result = json_encode(mysql_query("DELETE FROM GameTbl WHERE fb_id_a=" . $GLOBALS['least_fb_id'] . " AND fb_id_b=" . $GLOBALS['greatest_fb_id']));
